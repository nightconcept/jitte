<script lang="ts">
	...
	$effect(() => {
		...
		if (toLoad.length > 0) {
			...
			console.log(`[Recommendations] Loading ${toLoad.length} images for category "${selectedCategory}"`);
		}
	});
	...
	async function fetchRecommendations() {
		...
		try {
			...
			const data = await edhrecService.getCommanderRecommendations(commanderName);
			...
			data.cardlists.forEach((list) => {
				console.log(`[Recommendations] Processing list: ${list.header}, cards: ${list.cards.length}`);
			...
			console.log(`[Recommendations] Loaded ${recs.length} recommendations in ${categories.length} categories`);
	...
	function toggleCardSelection(cardName: string, rec: EDHRECCardRecommendation) {
	...
	async function addSelectedToDeck() {
		...
		for (const cardName of cardNames) {
			try {
				...
				console.error(`[Recommendations] Error adding card ${cardName}:`, error);
				failedCount++;
			}
		}
		...
		// Show result toast
		if (addedCount > 0) {
			toastStore.success(`Added ${addedCount} card${addedCount > 1 ? 's' : ''} to deck`, 5000);
		}
		if (failedCount > 0) {
			toastStore.error(`Failed to add ${failedCount} card${failedCount > 1 ? 's' : ''}`, 5000);
		}
	}
	...
	async function addSelectedToMaybeboard() {
		...
		// Show result toast
		if (addedCount > 0) {
			toastStore.success(`Added ${addedCount} card${addedCount > 1 ? 's' : ''} to maybeboard`, 5000);
		}
		if (failedCount > 0) {
			toastStore.error(`Failed to add ${failedCount} card${failedCount > 1 ? 's' : ''}`, 5000);
		}
...
<BaseModal {isOpen} {onClose} title="EDHREC Recommendations" subtitle={`For ${commanderName}`} size="custom" customSize="w-[95vw] max-w-[1800px]" height="h-[95vh]">
	<div class="flex flex-col h-full overflow-hidden">
		{#if isLoading}
			<div class="flex items-center justify-center h-full">
				<div class="text-center">
					<div class="animate-spin rounded-full h-12 w-12 border-b-2 border-[var(--color-brand-primary)] mx-auto"></div>
					<p class="mt-4 text-[var(--color-text-secondary)]">Loading recommendations from EDHREC...</p>
				</div>
			</div>
		{:else if categories.length === 0}
			<div class="flex items-center justify-center h-full">
				<div class="text-center">
					<p class="text-[var(--color-text-primary)] font-medium">No recommendations found</p>
		...
		{:else}
			...
			<div class="px-6 py-4 border-b border-[var(--color-border)] space-y-4">
				<div class="flex items-center gap-4 flex-wrap">
					...
					<div class="flex items-center gap-2">
						<label for="category-filter" class="text-sm font-medium text-[var(--color-text-secondary)]">Category:</label>
						<select
							id="category-filter"
					...
					<!-- Min Synergy Filter -->
					<div class="flex items-center gap-2">
						<label for="synergy-filter" class="text-sm font-medium text-[var(--color-text-secondary)]">Min Synergy:</label>
						<input
							id="synergy-filter"
							type="range"
							...
							class="w-32"
						/>
						<span class="text-sm text-[var(--color-text-primary)] font-medium w-12">{minSynergy}%</span>
					</div>
					...
					<label class="flex items-center gap-2 cursor-pointer">
						...
						/>
						<span class="text-sm font-medium text-[var(--color-text-secondary)]">Hide cards in deck</span>
					</label>
			...
			<div class="flex flex-1 overflow-hidden">
						...
						<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-7 gap-4">
							{#each filteredRecommendations() as rec (rec.name)}
								<div class="flex flex-col gap-2">
									<button
										class="relative group aspect-[5/7] rounded-lg overflow-hidden border-2 transition-all {selectedCards.has(rec.name)
									...
									>
										...
										<div class="absolute top-2 left-2 z-10">
											<div class="w-5 h-5 rounded bg-[var(--color-surface)] border-2 border-[var(--color-border)] flex items-center justify-center {selectedCards.has(rec.name) ? 'bg-[var(--color-brand-primary)] border-[var(--color-brand-primary)]' : ''}">
										...
										{:else if loadingCards.has(rec.name)}
											<!-- Loading spinner -->
											<div class="w-full h-full bg-[var(--color-bg-secondary)] flex flex-col items-center justify-center gap-3">
												<div class="animate-spin rounded-full h-8 w-8 border-b-2 border-[var(--color-brand-primary)]"></div>
												<div class="text-xs font-medium text-[var(--color-text-secondary)] text-center px-2 line-clamp-2">
													{rec.name}
												</div>
											</div>
										{:else}
											<!-- Placeholder - failed to load or not started -->
											<div class="w-full h-full bg-[var(--color-bg-secondary)] flex items-center justify-center p-3">
												<div class="text-sm font-medium text-[var(--color-text-primary)] line-clamp-3 text-center">
													{rec.name}
												</div>
											</div>
										{/if}
									...
									<div class="text-xs text-center space-y-0.5 px-1">
										<div class="font-semibold text-[var(--color-brand-primary)]">
				...
				<!-- Preview Panel -->
				<div class="w-80 border-l border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-6 overflow-y-auto">
					{#if hoveredCard}
						<div class="space-y-4">
							<!-- Card Image -->
							<div class="aspect-[5/7] rounded-lg overflow-hidden bg-[var(--color-bg-primary)] border border-[var(--color-border)]">
								{#if isLoadingPreview}
							...
							<!-- EDHREC Stats -->
							<div class="space-y-3">
								<h3 class="font-bold text-[var(--color-text-primary)]">{hoveredCard.name}</h3>

								<div class="space-y-2">
									<div class="flex justify-between items-center">
										<span class="text-sm text-[var(--color-text-secondary)]">Synergy Score</span>
										<span class="text-sm font-medium text-[var(--color-brand-primary)]">{hoveredCard.synergyScore}%</span>
									</div>
									<div class="flex justify-between items-center">
										<span class="text-sm text-[var(--color-text-secondary)]">Inclusion Rate</span>
										<span class="text-sm font-medium text-[var(--color-text-primary)]">{hoveredCard.inclusionRate}%</span>
									</div>
									<div class="flex justify-between items-center">
										<span class="text-sm text-[var(--color-text-secondary)]">Deck Count</span>
										<span class="text-sm font-medium text-[var(--color-text-primary)]">{hoveredCard.deckCount.toLocaleString()}</span>
									</div>
								</div>
			...
			<div class="px-6 py-4 border-t border-[var(--color-border)] flex items-center justify-between">
				...
				<div class="flex items-center gap-3">
					<button
						...
						class="px-4 py-2 text-sm bg-[var(--color-surface)] hover:bg-[var(--color-surface-hover)] border border-[var(--color-border)] rounded text-[var(--color-text-primary)] font-medium disabled:opacity-50 disabled:cursor-not-allowed"
					>
						Add to Maybeboard
					</button>
					<button
