...
>
  ...
  {#snippet content()}
    ...
    {#if gameChangers.length > 0}
      <div class="mb-2">
        <div
          class="font-semibold text-amber-500 mb-1 text-xs flex items-center gap-1"
        >
    ...
    {:else}
      <div class="text-xs bracket-secondary">No Game Changers detected</div>
    {/if}

    <!-- Criteria Info -->
    <div class="mt-2 pt-2 criteria-section text-xs">
      <div class="font-semibold bracket-description mb-1">
        Bracket Criteria:
      </div>
      {#if bracketLevel === 1}
        ...
        <div
          class={gameChangers.length > 0 ? "text-red-400" : "text-green-400"}
        >
      ...
      {:else if bracketLevel === 3}
        ...
        <div
          class={lateGameComboCount > 0 ? "text-amber-400" : "text-green-400"}
        >
          {lateGameComboCount > 0 ? "✓" : "○"} Late-game combos OK {lateGameComboCount >
          0
            ? `(${lateGameComboCount})`
            : ""}
        </div>
        <div
          class={gameChangers.length > 0 && gameChangers.length <= 3
            ? "text-amber-400"
            : gameChangers.length > 3
              ? "text-red-400"
              : "text-green-400"}
        >
          {gameChangers.length > 3 ? "✗" : gameChangers.length > 0 ? "✓" : "○"} Up
          to 3 Game Changers {gameChangers.length > 0
            ? `(${gameChangers.length})`
            : ""}
        </div>
      {:else if bracketLevel === 4}
        ...
        <div class="font-semibold bracket-description mb-1">
          Triggers (any applies):
        </div>
        <div
          class={hasMassLandDenial
            ? "text-orange-400 font-semibold"
            : "bracket-secondary"}
        >
          {hasMassLandDenial
            ? "! Mass land denial detected"
            : "○ Mass land denial"}
        </div>
        <div
          class={hasChainingExtraTurns
            ? "text-orange-400 font-semibold"
            : "bracket-secondary"}
        >
          {hasChainingExtraTurns
            ? "! Chaining extra turns detected"
            : "○ Chaining extra turns"}
        </div>
        <div
          class={earlyGameComboCount > 0
            ? "text-orange-400 font-semibold"
            : "bracket-secondary"}
        >
          {earlyGameComboCount > 0
            ? `! ${earlyGameComboCount} early-game ${earlyGameComboCount === 1 ? "combo" : "combos"} detected`
            : "○ Early-game 2-card combos"}
        </div>
        <div
          class={gameChangers.length >= 4
            ? "text-orange-400 font-semibold"
            : "bracket-secondary"}
        >
          {gameChangers.length >= 4
            ? `! ${gameChangers.length} Game Changers detected`
            : "○ 4+ Game Changers"}
        </div>
      {:else if bracketLevel === 5}
        <!-- cEDH: No restrictions -->
        <div class="text-red-400 font-semibold">! Competitive EDH (cEDH)</div>
        <div class="bracket-secondary">
          • No restrictions except banned list
        </div>
