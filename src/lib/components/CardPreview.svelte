<script lang="ts">
  ...
  $effect(() => {
    if (displayCard) {
      if (
        displayCard.layout &&
        (displayCard.layout === "modal_dfc" ||
          displayCard.layout === "transform")
      ) {
        ...
        if (!displayCard.cardFaces || displayCard.cardFaces.length < 2) {
          console.warn(
            "[CardPreview] ⚠️ MISSING DATA: This card needs cardFaces data. Re-add from search to enable flip functionality.",
          );
        }
      }
    }
...
<aside
  class="{isCollapsed
    ? 'w-12'
    : className ||
      'w-[28rem]'} bg-[var(--color-bg-secondary)] border-r border-[var(--color-border)] flex flex-col sticky top-[64px] self-start h-[calc(100vh-64px)] relative transition-all duration-200"
>
  <!-- Collapsible Header -->
  <button
  ...
  {#if !isCollapsed}
      ...
      <div class="flip-card" class:is-flipped={currentFaceIndex === 1}>
        ...
        <div class="card-face card-face--front">
          {#if isDoubleFaced && displayCard?.cardFaces?.[0]?.imageUrls}
            <div class="relative overflow-hidden rounded-lg">
              <img
                src={displayCard.cardFaces[0].imageUrls.normal ||
                  displayCard.cardFaces[0].imageUrls.large}
                alt={displayCard.cardFaces[0].name ||
                  displayCard?.name ||
                  "Card front"}
          ...
          {:else if displayCard}
            <div
              class="w-full aspect-[5/7] bg-[var(--color-surface)] rounded-lg flex items-center justify-center text-[var(--color-text-tertiary)]"
            >
        ...
        <div class="card-face card-face--back">
          {#if isDoubleFaced && displayCard?.cardFaces?.[1]?.imageUrls}
            <div class="relative overflow-hidden rounded-lg">
              <img
                src={displayCard.cardFaces[1].imageUrls.normal ||
                  displayCard.cardFaces[1].imageUrls.large}
    ...
    {#if showPricing && displayCard?.prices}
      <div class="pt-4 px-4 border-t border-[var(--color-border)]">
        <div
          class="text-xs font-semibold text-[var(--color-text-primary)] mb-3"
        >
          Prices
        </div>
        <div class="space-y-2">
