<script lang="ts">
	...
	let totalCards = $derived(categories.reduce((sum, cat) => sum + cat.cards.reduce((s, c) => s + c.quantity, 0), 0));
	...
	function toggleCardMenu(cardName: string) {
		openMaybeboardCardMenu = openMaybeboardCardMenu === cardName ? null : cardName;
	}
	...
	function saveQuantityEdit() {
		if (editQuantityCard) {
			deckStore.updateMaybeboardCardQuantity(editQuantityCard.name, activeCategory, editQuantityAmount);
		}
	...
	$effect(() => {
		function handleClickOutside(event: MouseEvent) {
			...
			if (categoryDropdownOpen && categoryDropdownRef && !categoryDropdownRef.contains(target)) {
				categoryDropdownOpen = false;
			}
		}
	...
	{#if categories.length > 0}
		<div class="flex items-center gap-2 mb-4">
			...
			<div class="relative flex-1" bind:this={categoryDropdownRef}>
				...
				{#if categoryDropdownOpen}
					<div class="absolute top-full mt-1 left-0 right-0 bg-[var(--color-surface)] border border-[var(--color-border)] rounded shadow-xl z-50 max-h-60 overflow-y-auto">
						{#each categories as category}
							<div class="flex items-center hover:bg-[var(--color-surface-hover)] {activeCategory === category.id ? 'bg-[var(--color-surface-hover)]' : ''}">
								<button
		...
		{#if showNewCategoryInput}
			<div class="flex items-center gap-2 mb-4">
				...
				<button
					onclick={() => { showNewCategoryInput = false; newCategoryName = ''; }}
		...
		{#if isEditing}
			<div class="mb-4 pb-4 border-b border-[var(--color-border)] space-y-3">
				<!-- Import Button -->
				<button
				...
				{#each cards as card}
					<div class="relative">
						...
						{#if openMaybeboardCardMenu === card.name}
							<div class="absolute right-0 top-full mt-1 w-48 bg-[var(--color-surface)] border border-[var(--color-border)] rounded shadow-xl z-[100]">
								<button
									onclick={(e) => { e.stopPropagation(); handleEditQuantity(card); }}
								...
								<button
									onclick={(e) => { e.stopPropagation(); handleMoveToDeck(card); }}
									class="w-full px-3 py-2 text-left text-sm hover:bg-[var(--color-surface-hover)] text-[var(--color-text-primary)] flex items-center gap-2"
								>
									<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	<div class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
		...
		>
			...
			<div class="px-6 py-4 border-b border-[var(--color-border)]">
				<h2 class="text-xl font-bold text-[var(--color-text-primary)]">Edit Quantity</h2>
				<p class="text-sm text-[var(--color-text-secondary)] mt-1">{editQuantityCard.name}</p>
			</div>
