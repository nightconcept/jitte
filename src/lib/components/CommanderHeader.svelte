<script lang="ts">
  ...
  $: commanderImageUrl =
    commander?.imageUrls?.artCrop || commander?.imageUrls?.large;
  $: bracketLabel = statistics?.bracketLevel
    ? getBracketLabel(statistics.bracketLevel)
    : "Unknown";
  ...
  $: typeDistribution = {
    planeswalker:
      deck?.cards[CardCategory.Planeswalker]?.reduce(
        (sum, c) => sum + c.quantity,
        0,
      ) || 0,
    creature:
      deck?.cards[CardCategory.Creature]?.reduce(
        (sum, c) => sum + c.quantity,
        0,
      ) || 0,
    instant:
      deck?.cards[CardCategory.Instant]?.reduce(
        (sum, c) => sum + c.quantity,
        0,
      ) || 0,
    sorcery:
      deck?.cards[CardCategory.Sorcery]?.reduce(
        (sum, c) => sum + c.quantity,
        0,
      ) || 0,
    artifact:
      deck?.cards[CardCategory.Artifact]?.reduce(
        (sum, c) => sum + c.quantity,
        0,
      ) || 0,
    enchantment:
      deck?.cards[CardCategory.Enchantment]?.reduce(
        (sum, c) => sum + c.quantity,
        0,
      ) || 0,
    land:
      deck?.cards[CardCategory.Land]?.reduce((sum, c) => sum + c.quantity, 0) ||
      0,
  };
...
<div class="h-28 flex-shrink-0 border-b border-[var(--color-border)]">
  <div class="relative h-full overflow-hidden">
    ...
    <div
      class="absolute inset-0 bg-[var(--color-bg-secondary)]/30 backdrop-blur-sm"
    ></div>
      ...
      <div class="min-w-[300px]">
        ...
        <div class="group flex items-center gap-2 h-8 mb-2">
          ...
          {:else}
            <!-- Display Mode -->
            <h1
              class="text-lg text-[var(--color-text-primary)] font-semibold leading-8"
            >
        ...
        <div
          class="flex items-center gap-4 text-sm text-[var(--color-text-secondary)]"
        >
          ...
          {#if deck && statistics?.bracketLevel}
            <div class="flex items-center gap-2">
              ...
              <BracketTooltip
                ...
                hasChainingExtraTurns={statistics?.hasChainingExtraTurns ??
                  false}
              >
                <span
