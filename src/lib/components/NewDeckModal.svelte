...
<BaseModal
	...
	subtitle={mode === 'empty' ? 'Start with an empty deck' : 'Import from decklist'}
	size={mode === 'empty' ? '2xl' : '4xl'}
	height={mode === 'import' ? 'h-[85vh]' : undefined}
>
	{#snippet children()}
		<!-- Body -->
		<div class="px-6 py-4 {mode === 'import' ? 'flex-1 flex flex-col min-h-0 overflow-y-auto' : 'space-y-4'}">
			<!-- Mode Selector -->
			<div class="mb-4">
				<div class="flex gap-2 p-1 bg-[var(--color-bg-primary)] rounded-lg border border-[var(--color-border)]">
					<button
						on:click={() => mode = 'empty'}
						class="flex-1 px-4 py-2 rounded-md text-sm font-medium transition-colors {mode === 'empty' ? 'bg-[var(--color-brand-primary)] text-white' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'}"
					>
						Start from Scratch
					</button>
					<button
						on:click={() => mode = 'import'}
						class="flex-1 px-4 py-2 rounded-md text-sm font-medium transition-colors {mode === 'import' ? 'bg-[var(--color-brand-primary)] text-white' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'}"
					>
						Import Decklist
					</button>
				</div>
		...
		<div class="px-6 py-4 border-t border-[var(--color-border)] flex justify-end gap-3">
			...
			<button
				...
				disabled={mode === 'empty' ? (!deckName.trim() || !selectedCommander) : (!deckName.trim() || selectedCommanders.length === 0)}
