  ...
  function selectPrinting(printing: ScryfallCard) {
    ...
    const newCard: Card = {
      ...
      imageUrls: {
        ...
        png:
          printing.image_uris?.png || printing.card_faces?.[0]?.image_uris?.png,
        artCrop:
          printing.image_uris?.art_crop ||
          printing.card_faces?.[0]?.image_uris?.art_crop,
        borderCrop:
          printing.image_uris?.border_crop ||
          printing.card_faces?.[0]?.image_uris?.border_crop,
      },
      ...
      {#if loading}
        <div class="flex items-center justify-center py-12">
          <div class="text-center">
            <div class="text-[var(--color-text-primary)] mb-2">
              Loading printings...
            </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {#each filteredPrintings as printing}
            <button
              ...
              class="group flex flex-col bg-[var(--color-bg-primary)] rounded-lg border-2 hover:border-[var(--color-brand-primary)] transition-all p-2 text-left {printing.id ===
              card.scryfallId
                ? 'border-[var(--color-brand-primary)] ring-2 ring-[var(--color-brand-primary)]/50'
            ...
            >
              ...
              <div class="relative overflow-hidden rounded mb-2">
                {#if printing.image_uris?.normal || printing.card_faces?.[0]?.image_uris?.normal}
                  ...
                  {#if printing.id === card.scryfallId}
                    <div
                      class="absolute top-2 right-2 bg-[var(--color-brand-primary)] text-white text-xs font-bold px-2 py-1 rounded"
                    >
                      CURRENT
                    </div>
                  {/if}
                {:else}
                  <div
                    class="w-full aspect-[5/7] bg-[var(--color-surface)] rounded flex items-center justify-center"
                  >
              ...
              <!-- Set Info -->
              <div
                class="text-xs font-bold text-[var(--color-text-primary)] mb-1"
              >
                {printing.set.toUpperCase()} #{printing.collector_number}
              </div>
              <div
                class="text-xs text-[var(--color-text-secondary)] mb-1 line-clamp-1"
