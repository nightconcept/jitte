<script lang="ts">
	...
	let sortBy = $state<'year' | 'price' | 'set'>('year');

	// Filtered and sorted printings
	let filteredPrintings = $derived(
		printings
			.filter((p) => {
	...
	$effect(() => {
		...
		async function fetchPrintings() {
			...
			try {
				printings = await cardService.getCardPrintings(oracleId);
				loading = false;
			} catch (err) {
...
>
	{#snippet children()}
		<!-- Search and Sort Controls (in header area) -->
		{#if !loading && !error && printings.length > 0}
			<div class="px-6 pb-4 border-b border-[var(--color-border)]">
