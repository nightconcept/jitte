...
export class CardCache {
	...
	async clearAll(): Promise<void> {
		const db = await this.getDb();
		const transaction = db.transaction([CARD_STORE, IMAGE_STORE, BULK_DATA_STORE], 'readwrite');
	...
	async getStats(): Promise<{
		...
		const db = await this.getDb();
		const transaction = db.transaction([CARD_STORE, IMAGE_STORE, BULK_DATA_STORE], 'readonly');

		const [cardCount, imageCount, bulkData] = await Promise.all([
